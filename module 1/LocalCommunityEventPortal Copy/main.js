...
async function fetchMockData() {
    ...
    if(eventsContainer) {
        eventsContainer.innerHTML = '<div class="text-center w-100 py-5"><div class="spinner-border" role="status"></div><p class="mt-2 text-muted">Loading events...</p></div>';
    }
        ...
        col.innerHTML = `
            <div class="card eventCard">
                ...
                <div class="position-relative">
                    ...
                    <span class="position-absolute top-0 end-0 bg-primary text-white px-2 py-1 m-2 rounded shadow-sm text-uppercase small fw-bold category-badge-${event.category}">${event.category}</span>
                </div>
                <div class="card-body">
                    <h5 class="card-title">${event.displayTitle}</h5>
                    <p class="card-text">${event.description}</p>
                    <ul class="list-unstyled">
                        <li><i class="fa-solid fa-calendar-days"></i> ${event.date}</li>
                        <li><i class="fa-solid fa-location-dot"></i> ${event.location}</li>
                        <li><i class="fa-solid fa-tag"></i> $${event.fee}</li>
                    </ul>
                    <!-- Requirement Completed: JS Exercise - Event Handling (onclick) -->
                    <button class="btn btn-outline btn-outline-primary w-100 mt-auto" onclick="openEventDetails(${event.id})">View Details</button>
                </div>
            </div>
        `;
